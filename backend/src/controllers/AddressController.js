...
const getAddressById = async (req, res) => {
  ...
  pool.query(
    "SELECT * FROM Address WHERE add_ID = $1",
    [id],
    (error, results) => {
      if (error) {
        console.error("Error executing query:", error);
...
const addAddress = async (req, res) => {
  const { email, latitude, longitude, minfo } = req.body;
  ...
  try {
    ...
    const query = `
          ...
          RETURNING *
      `;
    const values = [rows[0].user_id, latitude, longitude, minfo];
...
const deleteAddress = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM Address WHERE Add_ID = $1 RETURNING *",
      [id]
    );
